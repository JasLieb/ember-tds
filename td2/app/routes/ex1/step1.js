...
export default Route.extend({
  ...
  actions: {
   addToIncluded: function(source, dest, items){
    ...
    addAllToIncluded: function (source, dest) {
      dest.pushObjects(source)
      source.removeObjects(source);
    },
    removeAllFromIncluded: function(source, dest){
      dest.pushObjects(source)
      source.removeObjects(source);
    },
    removeFromIncluded: function(source, dest, items){
      let model=this.modelFor(this.routeName);
      dest.pushObjects(items);
      source.removeObjects(items);
      Ember.set(model,'dispoItemsIds_',[]);
